...
DATASET_DIR=${DATASET_DIR:-"/mnt/data3/work/coco"}
...
if [ ${NVTX_FLAG} -gt 0 ]; then
 NSYSCMD=" nsys profile --sample=none --cpuctxsw=none  --trace=cuda,nvtx  --force-overwrite true --output /results/ssd_${PGNNODES}x${PGNGPU}x${BATCHSIZE}_${DATESTAMP}_${SLURM_PROCID}.qdrep "
else
 NSYSCMD=""
fi
...
#    fi
  ...
  CMD=( ${NSYSCMD} "python" "-u" "-m" "bind_launch" "--nsockets_per_node=${PGNSOCKET}" \
