...
def get_filenames(is_training, data_dir):
  ...
  if is_training:
    ...
    return [
        os.path.join(data_dir, 'img_val/img_val-%05d-of-00256' % i)
        for i in range(_NUM_EVAL_FILES)]
...
def input_fn(is_training,
  ...
  if horovod_enabled():
    if is_training:
      cycle_length = _NUM_TRAIN_FILES // hvd.size()
    else:
      cycle_length = _NUM_EVAL_FILES // hvd.size()
