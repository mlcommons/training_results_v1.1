...
if [ ${NVTX_FLAG} -gt 0 ]; then
 NSYSCMD=" nsys profile --sample=none --cpuctxsw=none  --trace=cuda,nvtx  --force-overwrite true --output /results/mskrcnn_${DGXNNODES}x${DGXNGPU}x${BATCHSIZE}_${DATESTAMP}_${SLURM_PROCID}.qdrep "
else
 NSYSCMD=""
fi
...
if [ -n "${SLURM_LOCALID-}" ]; then
  ...
  if [ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]; then
    ...
    elif [[ "${DGXSYSTEM}" == R750*   ]]; then
        CMD=( './bind.sh' '--cpu=r750xa_topology.sh' '--mem=r750xa_topology.sh' '--' ${NSYSCMD} 'python' '-u' )
    elif [[ "${DGXSYSTEM}" == XE8545*   ]]; then
        CMD=( './bind.sh' '--cpu=xe8545_topology.sh' '--mem=xe8545_topology.sh' '--' ${NSYSCMD} 'python' '-u' )
    elif [[ "${DGXSYSTEM}" == DSS8440x8*   ]]; then
        CMD=( './bind.sh' '--cpu=dss8440x8_topology.sh' '--mem=dss8440x8_topology.sh' '--' ${NSYSCMD} 'python' '-u' )
    elif [[ "${DGXSYSTEM}" == DSS8440x10*   ]]; then
        CMD=( './bind.sh' '--cpu=dss8440x10_topology.sh' '--mem=dss8440x10_topology.sh' '--' ${NSYSCMD} 'python' '-u' )
    else
