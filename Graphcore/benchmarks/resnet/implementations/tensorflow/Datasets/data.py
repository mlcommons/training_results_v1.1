    ...
    if opts['generated_data'] or opts['synthetic_data']:
        ...
        if not is_training:
            ...
            count = (
                ...
                opts["validation_total_batch_size"])//opts['distributed_worker_count']
            # Size differ by up to 1 for the 128 files.
            ...
            dataset = dataset.apply(
                  tf.data.experimental.map_and_batch(
                    ...
                    drop_remainder=True  # not active since bs1
                  )
                )
            # unbatch for concatenate
            dataset = dataset.unbatch()
            padding_sample = generate_zero_sample(opts)
            # Fill up with zeros
            # Using cardinality of dataset instead does not work.
            dataset = dataset.concatenate(
                    padding_sample.repeat(count)).take(count)
