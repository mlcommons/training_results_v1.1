...
from utils.distributed import popdist_root, distributed_barrier, average_distributed_deques, sum_distributed_data, sum_distributed_dataf
...
def bert_optimizer_location_settings(args):
    ...
    locationSettings = popart.TensorLocationSettings(popart.TensorLocation(storage, rts))
...
def bert_session_patterns(args):
    ...
    if args.use_popdist:
        patterns.enablePattern("ZFusedReplicatedAllReducePattern", True)
    if args.task != "SQUAD":
    ...
    for iteration.epoch in range(start_epoch, iteration.epochs):
        for data in dataset:
                ...
                if (iteration.distributed and total_sequences_so_far) or (not iteration.distributed and per_instance):
                    ...
                    if args.accuracy_averaging_basis == "pertoken":
                        ...
                        if args.use_popdist:
                            ...
                            mlm_eval_accuracy = sum_distributed_dataf(mlm_eval_accuracy)
            ...
            if mlm_eval_accuracy >= mlm_accuracy_target or len(iteration.on_the_spot_validation_triggers) == 0:
                if not iteration.distributed: iteration.report_stats()
...
def main(args):
    ...
    mllogger.event(key="WEIGHTS_INITIALIZATION", metadata=dict(tensor="all"))
    ...
    if args.inference:
        ...
        if args.use_popdist:
            train_samples = sum_distributed_data(iteration.total_sequences_so_far)
