...
def generate_initializers(config, map_names, load_data, mapping, transform={}):
    ...
    for name, array in zip(map_names, load_data):
        ...
        if "Embedding/Embedding_Dict" in mapping[name]:
            ...
            diff = config.padded_vocab_length - tf_vocab_length
            # Pad or Crop the vocab.
            if diff > 0:
                logger.info(f"Padding the vocabulary. From {tf_vocab_length} to {config.padded_vocab_length}")
                pad = np.zeros((diff, config.hidden_size)).astype(array.dtype)
                array = np.concatenate((array, pad), axis=0)
            else:
                logger.warning(f"Cropping the vocabulary may negatively effect performance. From {tf_vocab_length} to {config.padded_vocab_length}")
                array = np.array(array[:config.padded_vocab_length, :])
        ...
        if "MLM/ProjectionB" in mapping[name]:
            ...
            if diff > 0:
                ...
                logger.warning(f"Cropping the projection bias from {tf_vocab_length} to {config.padded_vocab_length}")
                array = np.array(array[:config.padded_vocab_length])
