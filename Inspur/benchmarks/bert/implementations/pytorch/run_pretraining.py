...
#            assert use_DDP, "Running multi-GPU without --use_ddp"
        ...
        if time_tags:
            torch.cuda.synchronize()
            sbridge.print_tag(SBridge.INIT_TIME, time.time()-init_start, torch.distributed.get_rank())
        if nvtx_flag:
...
#                optimizer._init_everything()
            ...
            if time_tags:
                torch.cuda.synchronize()
                sbridge.add_tag(SBridge.LOAD_TIME, time.time()-loading_start, torch.distributed.get_rank())
            if nvtx_flag:
            ...
            for f_id in range(f_start_id + 1, len(files)):
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    if update_step:
                        ...
                        if time_tags:
                            torch.cuda.synchronize()
                            if not sbridge.add_tag(SBridge.OPT_TIME, time.time()-opt_start, torch.distributed.get_rank()):
                                sbridge.empty_qs()
                                time_tags = 0
                        if nvtx_flag:
                        ...
                        if (args.eval_dir and args.eval_iter_samples > 0 and global_step == next_eval_step):
                            ...
                            if time_tags:
                                torch.cuda.synchronize()
                                sbridge.add_tag(SBridge.EVAL_TIME, time.time()-eval_start, torch.distributed.get_rank())
                            if nvtx_flag:
            ...
            if time_tags or nvtx_flag:
                ...
                mlperf_logger.log_event(key=SBridge.EPOCH_TIME, value={'r':torch.distributed.get_rank(), 't':epoch_time}, log_all_ranks=True, sync=False)
        if time_tags:
            sbridge.empty_qs()
            time_tags = 0
