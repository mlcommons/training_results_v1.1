...
class SSDModel:
    ...
    def train_val(self, train_iterator, start_epoch=1, end_epoch=80,
        ...
        for epoch in range(start_epoch, end_epoch+1):
            ...
            ret = self.train_epoch(data_iterator=train_iterator,
                                   ...
                                   sbridge=sbridge, time_tags=time_tags,nvtx_flag=nvtx_flag)
            if ret > 0:
                return None, epoch
            ...
            if (val_interval and not epoch % val_interval) or (val_epochs and epoch in val_epochs):
                ...
                if nvtx_flag:
                    sbridge.push_nvtx(SBridge.EVAL_TIME)
                ...
                if time_tags:
                    mx.nd.waitall()
                    eval_iter = time.time() - eval_start
                    sbridge.add_tag(SBridge.EVAL_TIME, eval_iter, hvd.rank())
                if nvtx_flag:
                    sbridge.pop_nvtx()
    ...
    def train_epoch(self, data_iterator, global_train_batch_size, iterations_per_epoch,
                    ...
                    sbridge=None, time_tags=0, nvtx_flag=0):
        ...
        for i, (images, box_targets, cls_targets) in enumerate(data_iterator):
            ...
            sum_loss, sbridge = self.train_step(images=images, cls_targets=cls_targets, box_targets=box_targets, sbridge=sbridge, time_tags=time_tags, nvtx_flag=nvtx_flag)
            if time_tags:
                mx.nd.waitall()
                opt_start = time.time()
            if nvtx_flag:
            ...
            if time_tags:
                ...
                if not sbridge.add_tag(SBridge.OPT_TIME, opt_time, hvd.rank()):
                    sbridge.empty_qs()
                    time_tags = 0
            if nvtx_flag:
                sbridge.pop_nvtx()
        ...
        if time_tags or nvtx_flag:
            log_event(key=SBridge.EPOCH_TIME, value={'r':hvd.rank(), 't':epoch_time}, uniq=False)
    ...
    def train_step(self, images, cls_targets, box_targets, sbridge=None, time_tags=0, nvtx_flag=0):
        ...
        with autograd.record():
            ...
            if time_tags:
                mx.nd.waitall()
                bwd_start = time.time()
                fwd_time = bwd_start - fwd_start
                sbridge.add_tag(SBridge.FWD_TIME, fwd_time, hvd.rank())
            if nvtx_flag:
                sbridge.pop_nvtx()
            ...
            if time_tags:
                ...
                sbridge.add_tag(SBridge.BWD_TIME, bwd_time, hvd.rank())
            if nvtx_flag:
                sbridge.pop_nvtx()
        return sum_loss, sbridge
