...
def _log_print(logger, *args, **kwargs):
    ...
    the message.
    If 'log_rank' is set, then only that rank will print the logging
    message. If 'log_all_ranks' is also set, then every rank including
    this one will log the message
    """
    ...
    if kwargs.pop('log_all_ranks', False):
        ...
        log = (get_rank() == kwargs.get('log_rank', 0))

    if 'log_rank' in kwargs:
        del kwargs['log_rank']
