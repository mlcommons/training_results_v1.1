...
class FwdLossBwdTrainer():
    ...
    def step(self, step, batch, model, optimizer, sbridge=None, time_tags=0, nvtx_flag=0):
        loss = None
        mlm_acc = None
        ...
        if time_tags:
            torch.cuda.synchronize()
            bwd_start = time.time()
            sbridge.add_tag(SBridge.FWD_TIME, bwd_start-fwd_start,torch.distributed.get_rank())
        if nvtx_flag:
        ...
        if time_tags:
            torch.cuda.synchronize()
            sbridge.add_tag(SBridge.BWD_TIME, time.time()-bwd_start, torch.distributed.get_rank())
        if nvtx_flag:
