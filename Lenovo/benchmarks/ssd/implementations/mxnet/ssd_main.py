...
VALID_DATASETS = ['coco2017']
VALID_BACKBONES = ['resnet34_mlperf']
VALID_MODES = ['train']
...
def parse_args():
    ...
    parser.add_argument('--mode', type=str.lower, choices=VALID_MODES, default='train_val',
                        help='Mode to run; one of %s. currently has no effect' % VALID_MODES)
...
def main(async_executor=None):
    ...
    if args.async_val:
        ...
        def disable_loky_mem_check():
            from loky import process_executor
            process_executor._MEMORY_LEAK_CHECK_DELAY = 1e5
            process_executor._MAX_MEMORY_LEAK_SIZE = 5e12
        # disable loky's memory leak detection
        if hvd.rank() == 0:
            async_executor.submit(tag="dummy_fn", fn=disable_loky_mem_check)
    else:
    ...
    if hvd.rank() == 0:
        ...
        log_event(key='tracked_stats', value={'d_convergence': epoch}, metadata={'step': (epoch)})
    mx.nd.waitall()
    hvd.shutdown()
