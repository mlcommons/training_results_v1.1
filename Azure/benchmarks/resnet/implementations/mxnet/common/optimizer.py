...
from mxnet.ndarray import (NDArray, zeros, clip, sqrt, cast, maximum, abs as NDabs, array, multiply, where,multi_sum_sq, multi_lars_v2, broadcast_greater_equal, broadcast_mul, broadcast_div, broadcast_sub, broadcast_add, broadcast_power)
...
class SGDwFASTLARSV2(Optimizer):
    ...
    def __init__(self, base_lr, end_lr, lr_decay_poly_power, 
            ...
            #cur_step_buffer, next_step_buffer,
            #base_momentum_buffer, scaled_momentum_buffer,
            #new_lrs_buffer, old_lrs_buffer,
            #poly_lrs_buffer, old_poly_lrs_buffer,
            #new_wds_buffer, sgd_wds_buffer,
            #w_sum_sq_buffer, g_sum_sq_buffer,
            #new_weights_buffer,
            #new_states_buffer,
            #new_states_weight_buffer, #master weights
            #new_grads_buffer,
        ...
        self.old_poly_lrs = mx.nd.array([0.0] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        ...
        #self.base_lr_gpu = mx.nd.array([self.base_lr] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.end_lr_gpu = mx.nd.array([self.end_lr] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.warmup_steps_gpu = mx.nd.array([self.warmup_steps] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.non_warmup_steps_gpu = mx.nd.array([self.total_steps - self.warmup_steps] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.steps_gpu = mx.nd.array([0.0] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.steps_gpu = mx.nd.array([0.0] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.lr_decay_poly_power_gpu = mx.nd.array([self.lr_decay_poly_power] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        self.ones_gpu = mx.nd.array([1.0] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.diff_lr_gpu = mx.nd.array([self.end_lr - self.base_lr] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.cond_gpu = mx.nd.array([1.0] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.w_poly_lrs = mx.nd.array([1.0] * self.full_index, ctx=mx.gpu(hvd.local_rank()), dtype='float32')
        #self.new_weights = new_weights_buffer
        #self.new_states = new_states_buffer
        #self.new_states_weight = new_states_weight_buffer
        #self.new_grads = new_grads_buffer
    ...
    def _update_impl(self, indices, weights, grads, states, multi_precision=False):
        ...
        if aggregate:
            ...
            if self.lars and len(lars_idx) > 0:
                ...
                # scaled momentum is not needed anymore to be compliant with the refrence 
                #self.scaled_momentum[:nb_lars] = where(self.old_poly_lrs[:nb_lars] > 0.0, self.base_momentum[:nb_lars] * self.poly_lrs[:nb_lars] / self.old_poly_lrs[:nb_lars] ,self.base_momentum[:nb_lars]);
                ...
                # scaled momentum is not needed anymore to be compliant with the refrence 
                #self.scaled_momentum[nb_lars:] = self.scaled_momentum[:len(full_idx)-nb_lars]
