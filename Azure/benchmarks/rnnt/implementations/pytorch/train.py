...
def train_step( model, loss_fn, args, batch_size, feats, feat_lens, txt, txt_lens, optimizer, grad_scaler, 
                meta_data, train_loader, rnnt_graph, copy_stream, pred_stream, sbridge=None, time_tags=0,
                nvtx_flag=0):

    fwd_start = fbwd_start = bwd_start = 0
    if time_tags:
    ...
    if args.batch_split_factor == 1:
        ...
        if time_tags:
            torch.cuda.synchronize()
            sbridge.add_tag(SBridge.BWD_TIME, time.time()-bwd_start, dist.get_rank())
        if nvtx_flag:
            sbridge.pop_nvtx()#bwd_time
        ...
        if time_tags:
            torch.cuda.synchronize()
            fbwd_start = time.time()
            sbridge.add_tag(SBridge.FWD_TIME, fbwd_start-fwd_start, dist.get_rank())
        if nvtx_flag:
            sbridge.pop_nvtx()#fwd_time
            sbridge.push_nvtx(SBridge.FBWD_TIME)
        ...
        if time_tags:
            torch.cuda.synchronize()
            bwd_start = time.time()
            sbridge.add_tag(SBridge.FBWD_TIME, bwd_start-fbwd_start, dist.get_rank())
        if nvtx_flag:
            sbridge.pop_nvtx()#fbwd_time
            sbridge.push_nvtx(SBridge.BWD_TIME)
...
def main():
    ...
    logging.log_event(logging.constants.SUBMISSION_ORG, value='Azure')
    logging.log_event(logging.constants.SUBMISSION_DIVISION, value=logging.constants.CLOSED) # closed or open
    logging.log_event(logging.constants.SUBMISSION_STATUS, value=logging.constants.CLOUD) # on-prem/cloud/research
    logging.log_event(logging.constants.SUBMISSION_PLATFORM, value=f'{num_nodes}xND96amsr_A100_v4')
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        if time_tags or nvtx_flag:
            logging.log_event(key=SBridge.EPOCH_TIME, value={'r':dist.get_rank(), 't':epoch_time})
