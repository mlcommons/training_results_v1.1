...
def main():
    ...
    time_tags, nvtx_flag, sbridge, load_start = init_bridge()
    if nvtx_flag:
        sbridge.push_nvtx(SBridge.LOAD_TIME)
    ...
    if time_tags:
        mx.nd.waitall()
        sbridge.print_tag(SBridge.LOAD_TIME, time.time() - load_start, global_rank)
    if nvtx_flag:
        sbridge.pop_nvtx()#SBridge.LOAD_TIME
