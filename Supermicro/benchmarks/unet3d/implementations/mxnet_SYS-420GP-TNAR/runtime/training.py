...
def train(flags, model, train_loader, val_loader, score_fn, sw_inference, comm, train_comm, eval_comm,
    ...
    time_tags, nvtx_flag, sbridge, epoch_start = init_bridge()
    ...
    for cycle in range(1, max_cycles):
                ...
                for i, batch in enumerate(tqdm(train_loader, disable=(rank != 0) or not flags.verbose)):
                    ...
                    with autograd.record():
                        ...
                        if time_tags:
                            mx.nd.waitall()
                            bwd_start = time()
                            sbridge.add_tag(SBridge.FWD_TIME, bwd_start-fwd_start, rank)
                        if nvtx_flag:
                            sbridge.pop_nvtx()#SBridge.FWD_TIME
                        ...
                        if time_tags:
                            mx.nd.waitall()
                            sbridge.add_tag(SBridge.BWD_TIME, time()-bwd_start, rank)
                        if nvtx_flag:
                            sbridge.pop_nvtx()# SBridge.BWD_TIME
                    opt_start = 0
                    if time_tags:
                        mx.nd.waitall()
                        opt_start = time()
                    ...
                    if time_tags:
                        mx.nd.waitall()
                        if not sbridge.add_tag(SBridge.OPT_TIME, time()-opt_start, rank):
                            sbridge.empty_qs()
                            time_tags = 0
                    if nvtx_flag:
                        sbridge.pop_nvtx()# SBridge.OPT_TIME
        ...
        if (global_epoch >= flags.start_eval_at) and flags.nodes_for_eval:
            ...
            if time_tags:
                mx.nd.waitall()
                sbridge.add_tag(SBridge.EVAL_TIME, time()-eval_start, rank)
            if nvtx_flag:
                sbridge.pop_nvtx()#SBridge.EVAL_TIME)
    ...
    if time_tags or nvtx_flag:
        sbridge.empty_qs()
        mx.nd.waitall()
        mllog_event(key=SBridge.EPOCH_TIME, value={'r': rank, 't': time()-epoch_start}, uniq=False)
