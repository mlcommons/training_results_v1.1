...
def make_val_split_even(x_val, y_val, num_shards, shard_id, shard_eval, batch_size, local_shard_size):
    ...
    if shard_id == 0:
        print(f"Split time {time()-t0}, curr_work_per_shard {curr_work_per_shard}, total work {sum(work)}, num_shards {num_shards}")
...
def get_data_split(path: str):
    ...
    # rnd_idx = np.random.permutation(len(imgs_train))
    # imgs_train = np.array(imgs_train)[rnd_idx].tolist()
    # lbls_train = np.array(lbls_train)[rnd_idx].tolist()
...
def get_data_loaders(flags, data_dir, seed, local_rank, global_rank, train_ranks, eval_ranks,
    ...
    if global_rank in train_ranks:
        ...
        if flags.stick_to_shard:
            ...
            # print(f"SHARD {shard_id}, len {len(x_train)}, x_train {(x_train[0], x_train[-1])}")
            num_shards = 1
            shard_id = 0
            dataset_len = len(x_train)
    ...
    if global_rank in eval_ranks:
        ...
        print(f"RANK {global_rank} SHARDID {shard_id} NUM_SHARDS {num_shards}")
        ...
        # x_val, y_val = [x_val[0]], [y_val[0]] # TODO: remove that
