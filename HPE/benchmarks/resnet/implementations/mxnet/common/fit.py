...
def mlperf_fit(self, args, train_data,
    ...
    for epoch in range(begin_epoch, num_epoch):
        if args.profile > 0:
            cu.nvtx_range_push("epoch{}".format(epoch))
        ...
        while not end_of_batch:
            if args.profile > 0:
                cu.nvtx_range_push("iter{}".format(nbatch))
            if nbatch % 2 == 0:
            ...
            nbatch += 1
            if args.profile > 0:
                cu.nvtx_range_pop() #pop iteration

        if args.profile > 0:
            cu.nvtx_range_pop() # pop epoch
        ...
        if eval_data and epoch % eval_period == eval_offset and not args.profile > 0:
            mx.ndarray.waitall() 
            if args.profile > 0:
                cu.nvtx_range_push("eval{}".format(epoch))
            mx_resnet_print_start(key=constants.EVAL_START, metadata={'epoch_num': epoch + 1})
            reduce_batchnorm_stats(self)
            if args.profile > 0:
                cu.nvtx_range_push("score{}".format(epoch))
...
def reduce_batchnorm_stats(module, in_place=True):
    ...
    if (in_place) :
        tensor = []
        for i in range(0,len(module._exec_group.aux_arrays[0])):
            tensor.append(module._exec_group.aux_arrays[0][i])
