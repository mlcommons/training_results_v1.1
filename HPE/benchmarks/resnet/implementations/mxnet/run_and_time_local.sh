...
if [[ ${PROFILE} -ge 1 ]]; then
        ...
        if [[ ${local_rank} == 0 || ${PROFILE_ALL_LOCAL_RANKS} == 1 ]]; then
            echo "OMPI_COMM_WORLD_LOCAL_RANK=${OMPI_COMM_WORLD_LOCAL_RANK} SLURM_LOCALID=${SLURM_LOCALID} LOCAL_RANK=${LOCAL_RANK} SLURM_PROCID=${SLURM_PROCID} SLURM_NODEID=${SLURM_NODEID}"
            #PROFILE_COMMAND="nsys profile --capture-range cudaProfilerApi --stop-on-range-end true --sample cpu --trace=cuda,nvtx --force-overwrite true --export=sqlite --output /results/${NETWORK}_b${BATCHSIZE}_%h_${local_rank}_${global_rank}.qdrep "
            PROFILE_COMMAND="nsys profile --capture-range cudaProfilerApi --stop-on-range-end true --sample cpu --trace=cuda,nvtx --force-overwrite true --export=sqlite --output /results/${NETWORK}_b${BATCHSIZE}_%h_${local_rank}_${global_rank}_${RANDOM}.qdrep "
            if [[ ${PROFILE_MPI} -ge 1 ]]; then
                echo "enable --trace=mpi"
                PROFILE_COMMAND="nsys profile --capture-range cudaProfilerApi --stop-on-range-end true --sample cpu --trace=cuda,nvtx,mpi --force-overwrite true --export=sqlite --output /results/${NETWORK}_b${BATCHSIZE}_%h_${local_rank}_${global_rank}_${SLURM_PROCID}.qdrep "
            fi
        fi
    #fi
fi
...
if [[ ${PROFILE} -ge 1 ]]; then
    TMPDIR=/results ${PROFILE_COMMAND} ${DISTRIBUTED} ${BIND} python train_imagenet.py "${PARAMS[@]}"; ret_code=$?
else
    ${LOGGER:-} ${DISTRIBUTED} ${BIND} ${NSYSCMD} ${GDB} python train_imagenet.py "${PARAMS[@]}"; ret_code=$?
fi
